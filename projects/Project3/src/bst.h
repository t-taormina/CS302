...
class Tree
{
    public:
        ...
        int insert(const shared_ptr<Concept>& Cptr);
        int display();
        int remove(const shared_ptr<Concept>& ptr);
        int add_language(shared_ptr<Concept> ptr, string& to_add);
        int remove_all();

    private:
        int insert(const shared_ptr<Concept>& Cptr, shared_ptr<Node>& curr);
        int display(shared_ptr<Node> curr);
        int remove(const shared_ptr<Concept>& ptr, shared_ptr<Node>& curr, int& flag);
